...
class SimMatch_Net(nn.Module):
    def __init__(self, base, proj_size=128, shared=False):
        super(SimMatch_Net, self).__init__()
        self.backbone = base
        if shared:
            self.feat_planes = 1024
        else:
    ...
    def forward(self, x, **kwargs):
        logits, _, feat = self.backbone(x, feature=True)
...
def main():
    ...
    if args.both_head_increment == 'shared' or args.both_head_increment == 'shared2':
        sh = True
    else:
        sh = False
    model = SimMatch_Net(model, proj_size=128, shared=sh)
