"""
FixMatch training + SSB training
"""
import logging
import time
import copy
...
def train(args, labeled_trainloader, unlabeled_dataset, test_loader, val_loader,
    ...
    if args.local_rank in [-1, 0]:
        ...
        logger_custom.set_names(['train_loss', 'train_loss_x', 'train_loss_o', 'train_loss_o_u', 'train_loss_oem',
                                 ...
                                 'total_acc', 'Mask', 'Prec', 'Recall', 'Used_acc', 'Used OOD', 'mask_neg_percent_ova', 'Used_neg_prec_ova',
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            if epoch >= args.start_fix:
                ...
                targets_id = (targets_u_gt < args.num_classes).float().cpu().numpy()  # ID index
                prec_id.update(precision_score(targets_id, pred_id))
                recall.update(recall_score(targets_id, pred_id))
            else:
        ...
        if args.local_rank in [-1, 0]:
            ...
            logger_custom.append(
                [losses.avg, losses_x.avg, losses_o.avg, losses_o_u.avg, losses_oem.avg, losses_socr.avg, losses_fix.avg,
                 total_c.avg, mask_probs.avg, prec_id.avg, recall.avg, used_c.avg, used_ood.avg, mask_neg_percent_ova.avg, used_neg_prec_ova.avg,
