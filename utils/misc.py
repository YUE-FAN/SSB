...
def f1_score(pred, target, avg):
    """Make sure OOD class index is the last one"""
    assert len(pred.shape) == 1
    assert len(target.shape) == 1

    cm = confusion_matrix(target, pred)
    tp = np.diag(cm)[:-1]  # the last one is OOD class
    tp_fp = cm.sum(axis=0)[:-1]
    tp_fn = cm.sum(axis=1)[:-1]
    if avg == 'micro':
        prec = tp.sum() / tp_fp.sum()
        recall = tp.sum() / tp_fn.sum()
    elif avg =='macro':
        prec = (tp / tp_fp).mean()
...
def ova_loss(logits_open, label):
    logits_open = logits_open.view(logits_open.size(0), 2, -1)  # [bs, 2, num_class]
    logits_open = F.softmax(logits_open, 1)
    label_s_sp = torch.zeros((logits_open.size(0),
                              logits_open.size(2))).long().to(label.device)  # [bs, num_class]
...
def exclude_dataset(args, dataset, model, exclude_known=False):
    print(args.analysis_increment_exclude_unlabeled_threshold)
    ...
    with torch.no_grad():
        if args.return_idx:
            for batch_idx, ((_, _, inputs), targets, _) in enumerate(test_loader):
                ...
                known_ind = unk_score < args.analysis_increment_exclude_unlabeled_threshold
                if batch_idx == 0:
                    known_all = known_ind
                else:
        ...
        if not args.no_progress:
            test_loader.close()
    print(known_all.float().sum())
    known_all = known_all.data.cpu().float().numpy()
    if exclude_known:
...
def test(args, test_loader, model, epoch, val=False):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            ...
            outputs, outputs_open = model(inputs)  # [bs, num_class], [bs, 2 * num_class]
            ...
            # f1_micro = f1_score(pred_close.data.cpu().numpy(), targets.data.cpu().numpy(), 'micro')
            # f1_macro = f1_score(pred_close.data.cpu().numpy(), targets.data.cpu().numpy(), 'macro')
            # f1_mi.update(f1_micro.item())
            # f1_ma.update(f1_macro.item())
